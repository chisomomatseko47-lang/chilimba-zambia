...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!NETWORKS.has(network)) return NextResponse.json({ error: 'Choose MTN, Airtel or Zamtel.' }, { status: 400 });
    ...
    const { data: tx, error } = await supabase.from('payment_transactions').insert({ user_id: user.id, group_id: contribution.group_id, direction: 'inbound', status: result.status, provider, provider_transaction_id: result.reference, idempotency_key: result.reference, amount: contribution.amount, currency: 'ZMW', metadata: { contribution_id: contribution.id, phone, network } }).select('id').single();
    ...
    return NextResponse.json({ ok: true, transaction_id: tx.id, reference: result.reference, status: result.status, provider, network });
